...
void matroska_writer::write_header()
{
    ...
    matroska_crc32_store(buffer + matroska_writer_header_tracks_crc_beg, matroska_writer_header_tracks_crc_end - matroska_writer_header_tracks_crc_beg);
...
void matroska_writer::write_frame(const char* video_buffer, int video_size, const char* audio_buffer, int audio_size, TimeCode timecode)
{
    ...
    if (timecode.IsValid())
    {
        ...
        store_e4(cur, 1 + 4 + 1 + 2 + 1 + video_size + (timecode.IsValid() ? 18 : 0));
